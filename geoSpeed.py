...
def readCSV():
    output_data = []
    with open("N_magnetic_pole.csv") as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                coords = row[1].split(" ")
                current_coords = [float(coord) for coord in coords]
                year = int(row[0])
                ...
                if previous_coords is not None and previous_year is not None:
                    ...
                    output_data.append([previous_year, year, round(distance / 1000, 2), round(speed, 2)])

                previous_coords = current_coords
                previous_year = year
                line_count += 1

        with open("distance_speed.csv", mode="w", newline="") as output_file:
            csv_writer = csv.writer(output_file)
            csv_writer.writerow(["Start Year", "End Year", "Distance (km)", "Speed (km/yr)"])
